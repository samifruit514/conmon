...
void *healthcheck_timer_thread(void *user_data)
{
	...
	while (local_timer->timer_active) {
		...
		if (local_timer->start_period_remaining > 0) {
			...
			if (local_timer->start_period_remaining > 0) {
				...
				if (local_timer->status != HEALTHCHECK_STARTING) {
					...
					ninfof("Healthcheck status changed to: starting (startup period: %d seconds remaining)",
					       local_timer->start_period_remaining);
		...
		if (exit_code == 0) {
			...
			if (local_timer->start_period_remaining > 0) {
				ninfof("Healthcheck failed during startup period (exit code: %d) - not counting against retry limit",
				       exit_code);
				...
				if (local_timer->consecutive_failures >= local_timer->config.retries) {
					local_timer->status = HEALTHCHECK_UNHEALTHY;
					ninfof("Healthcheck status changed to: unhealthy (exit code: %d, retries: %d)", exit_code,
					       local_timer->consecutive_failures);
					healthcheck_send_status_update(local_timer->container_id, local_timer->status, exit_code);
				} else {
					ninfof("Healthcheck failed (exit code: %d), consecutive failures: %d/%d", exit_code,
					       local_timer->consecutive_failures, local_timer->config.retries);
				}
			}
		}
