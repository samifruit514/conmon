...
@test "healthcheck CLI options are parsed correctly" {
    # Test that healthcheck CLI options are properly parsed
    run $CONMON_BINARY --bundle /tmp --cid test --cuuid test --runtime /bin/true --healthcheck-cmd "echo healthy" --healthcheck-interval 30 --healthcheck-timeout 10 --healthcheck-retries 3 --healthcheck-start-period 0 --version
    [ "$status" -eq 0 ]
}
...
@test "healthcheck with minimal required arguments" {
    ...
    run $CONMON_BINARY --bundle /tmp --cid test --cuuid test --runtime /bin/true --healthcheck-cmd "echo healthy" --version
    [ "$status" -eq 0 ]
}

@test "healthcheck with all parameters specified" {
    # Test healthcheck with all parameters specified
    run $CONMON_BINARY --bundle /tmp --cid test --cuuid test --runtime /bin/true --healthcheck-cmd "curl -f http://localhost:8080/health" --healthcheck-interval 60 --healthcheck-timeout 30 --healthcheck-retries 5 --healthcheck-start-period 120 --version
    [ "$status" -eq 0 ]
}

@test "healthcheck parameters accept valid values" {
    # Test that healthcheck parameters accept reasonable values
    run $CONMON_BINARY --bundle /tmp --cid test --cuuid test --runtime /bin/true --healthcheck-cmd "echo test" --healthcheck-interval 1 --healthcheck-timeout 1 --healthcheck-retries 1 --healthcheck-start-period 0 --version
    [ "$status" -eq 0 ]
}
...
EOF
    ...
    # Test healthcheck with a short-lived container
